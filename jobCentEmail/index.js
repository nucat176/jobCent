...
gmailApiSync.setClientSecretsFile('');
...
let tkn = {access_token: '',
  refresh_token: '',
  scope: 'https://mail.google.com/ https://www.googleapis.com/auth/gmail.modify https://www.googleapis.com/auth/gmail.readonly https://www.googleapis.com/auth/gmail.send',
  token_type: 'Bearer',
...
function initJobCent() {
	...
	new Promise(function(resolve, reject) {
		return jobcentSdkInstance.storeWalletAddress('jobcent@ncnt.io', jobcentKeyPair, resolve, reject);
	})
	.then(function() {
		new Promise(function(resolve, reject) {
			return ncentSdkInstance.stampToken(jobcentKeyPair.publicKey(), 'Jbcent', 1000000, '2021', resolve, reject);
		})
		...
		.then(function() {
			return createAndTransfer('', 2000);
		})
		.then(function() {
			return createAndTransfer('', 2000);
		})
		.then(function() {
			return createAndTransfer('', 1000);
		})
		.then(function() {
			return createAndTransfer('', 2000);
		})
		.then(function() {
			return createAndTransfer('', 1);
		})
		.then(function() {
			return createAndTransfer('', 1);
		})
		...
		.then(function(createWalletResponse) {
			...
			.then(function(response) {
				...
				if (createWalletResponse !== "nothing done") {
					return new Promise(function(resolve, reject) {
						ncentSdkInstance.transferTokens(jobcentKeyPair, response.data[0].public_key, token_id, amount, resolve, reject);
					})
				} else {
					return new Promise(function(resolve, reject) {
						ncentSdkInstance.getTokenBalance(response.data[0].public_key, token_id, resolve, reject);
					})
				}
			})
			.catch(function(error) {
		...
		if (response.data.txn || (response.data[0].balance !== 0)) {
			...
			.then(function() {
				...
				if (to !== 'jobcent@ncnt.io') {
					...
					.then(function(resp) {
						sender_keyPair = StellarSdk.Keypair.fromSecret(resp.data[0].private_key);
					})
					.then(function() {
						new Promise(function(resolve, reject) {
						...
						.then(function(rsp) {
							new Promise(function(resolve, reject) {
								ncentSdkInstance.transferTokens(sender_keyPair, rsp.data[0].public_key, token_id, 1, resolve, reject);
							})
							.then(function() {
